...
import { EmbeddingDto } from './dto/Embeddings/embeddings.dto';
import { UploadFileDto, ListFilesDto, RetrieveFileDto, DeleteFileDto, DownloadFileDto } from './dto/Files/files.dto';
import { AgentsCompletionDto, AgentsCompletionStreamDto } from './dto/Agents/agents.dto';
import { Mistral } from '@mistralai/mistralai';
import { ArchiveFineTunedModelDto, DeleteModelDto, ListModelsDto, RetrieveModelDto, UnarchiveFineTunedModelDto, UpdateFineTunedModelDto } from './dto/Models/models.dto';
import { Observable } from 'rxjs';
import { FIMCompletionDto, FIMCompletionStreamDto } from './dto/Fim/fim.dto';
import { CancelBatchJobDto, CreateBatchJobDto, GetBatchJobDto, ListBatchJobDto } from './dto/Batch/batch-jobs.dto';
import { CancelFineTuningJobDto, CreateFineTuningJobDto, GetFineTuningJobDto, ListFineTuningJobsDto, StartFineTuningJobDto } from './dto/FineTuning/finetuning-jobs.dto';
import { ChatModerationDto, ContentModerationDto } from './dto/Classifiers/classifiers.dto';
...
export class MistralService {
  ...
  chatCompletionStream(chatStream: ChatCompletionStreamDto): Observable<string> {
    ...
    return new Observable<string>((observer) => {
      (async () => {
        try {
          ...
          this.logger.error(`Error while processing FIM stream: ${error.message}`);
          observer.error(error);
        }
      })();
    });
  ...
  agentsCompletionStream(agentsCompletionStream: AgentsCompletionStreamDto): Observable<string> {
    ...
    return new Observable<string>((observer) => {
      (async () => {
        try {
          ...
          const eventStream = await this.mistral.agents.stream(request, options);
          ...
          this.logger.error(`Error while processing agents completion stream: ${error.message}`);
  ...
  async moderateContent(contentModeration: ContentModerationDto) {
    ...
    try {
      const moderation = await this.mistral.classifiers.moderate(request, options );
  ...
  async moderateChat(chatModeration: ChatModerationDto) {
    ...
    try {
      const moderation = await this.mistral.classifiers.moderateChat(request, options);
